...
void Gamebay::setUI(){
    ...
    //!!!!!! A COMPLETER !!!!!!
    QGridLayout* gridAttaques = new QGridLayout;
    gridAttaques->addWidget(choixAttaque_->attaque1_, 0, 0);
    gridAttaques->addWidget(choixAttaque_->attaque2_, 0, 1);
    gridAttaques->addWidget(choixAttaque_->attaque3_, 1, 0);
    gridAttaques->addWidget(choixAttaque_->attaque4_, 1, 1);
    ...
    layoutEcran->addLayout(gridAttaques);
...
void Gamebay::attaquerCreatureAdverse(){
    //Cette methode va permettre de voir les consequences de votre attaque sur la creature adverse
     ...
     int compteur = ExceptionCreatureMorte::obtenirValeurCompteur();
     ExceptionAttaqueEchouee echoue;
     ExceptionCreatureMorte morte;
     bool creatureBattue = false;
     ...
     catch(ExceptionAttaqueEchouee echou){
         QString messageErreur("l'attaque a echouee");
         QMessageBox::critical(this,messageErreur,"Erreur !");
     }

     catch(ExceptionCreatureMorte morte) {
         QString titre("Creature adverse est deja morte");
         QString message("");
         if(compteur < 3)
             message = "Arretez c'est deja fini!";
         else if(compteur < 5)
             message = "Hey ca fait deja 3e fois que vous esasyez...";
         else if(compteur > 5)
             message = "Vous dis donc... Vous etes vraiment sadique a vous acharner sur une pauvre creature deja morte";

        QMessageBox::critical(this, message, titre);
...
void Gamebay::capturerCreatureAdverse(){
    //!!!!!! A COMPLETER !!!!!!
    QMessageBox msg;
    ExceptionEchecCapture echec;

    try{
    polyland_->attraperCreature(&polyland_->obtenirHero(), creatureAdverse_);
    if(polyland_->obtenirHero().obtenirUneCreature(creatureAdverse_->obtenirNom()) == creatureAdverse_)
        throw echec;
    }
    catch(ExceptionEchecCapture echec){
        QString message("vous possedez deja cette creature");
        QString titre("Capture impossible");
        QMessageBox::critical(this, message, titre);

...
int main()
{
	...
	Dresseur* Sasha2 = new Dresseur("Sasha");
	...
	Pokachu.information();
	Pokachu.obtenirPouvoir().description();
	...
	polyland.ajouterDresseur(Sasha2);
	...
	if (polyland.attraperCreature(&Vous, Salimouche)) {
		...
		cout << "ERREUR : Le salimouche s'est échappé" << endl;
	}
	...
	if (polyland.attraperCreature(&Vous, *creatureAleatoire)) {
		//ce cas ne devrait pas s'afficher
		cout << "ERREUR : Felicitation vous avez attrapé un " << creatureAleatoire->obtenirNom() << " !" << endl;
	}
	else {
		//Elle s'échappe donc
	...
	if (polyland.relacherCreature(&Vous, Salimouche)) {
		...
		//ce cas ne devrait pas s'afficher
		cout << "ERREUR : Oh il y a eu un petit problème technique... Salimouche n'a pas pu être relaché" << endl;
	}
	...
	if (polyland.retirerDresseur(dresseurAleatoire->obtenirNom())){
		...
		//ce cas ne devrait pas s'afficher
		cout << "ERREUR : Oh il y a eu un petit problème technique... " << dresseurAleatoire->obtenirNom() << "n'a pas pu s'enfuir" << endl;
	}
